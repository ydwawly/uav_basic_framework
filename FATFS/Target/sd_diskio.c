...
// __attribute__((section(".dma_buffer"), aligned(32))) static uint8_t dma_scratch_buff[512];
/* 放在普通的 RAM_D1 里，编译器自动分配，绝不乱丢。
 * 强制 32 字节对齐，完美配合 Cache 硬件清洗指令！ */
__attribute__((aligned(32))) static uint8_t dma_scratch_buff[512];
...
DRESULT SD_read(BYTE lun, BYTE *buff, DWORD sector, UINT count)
{
  ...
  {
    /* 1. 防脏数据：清洗 Cache */
    SCB_CleanInvalidateDCache_by_Addr((uint32_t*)dma_scratch_buff, 512);

    /* 2. 启动 DMA */
    if(BSP_SD_ReadBlocks_DMA((uint32_t*)dma_scratch_buff, sector + i, 1) == MSD_OK) {

      /* 3. 任务休眠等待 */
      if(osMessageQueueGet(SDQueueID, &msg, NULL, 1000) == osOK) {

        /* 🚨🚨🚨 【最核心的查错防线】 🚨🚨🚨 */
        if (HAL_SD_GetError(&hsd1) != HAL_SD_ERROR_NONE) {
          // DMA 确实是被唤醒了，但它是报错夭折的！
          // 如果进到这里，说明您的 SDMMC 时钟还是太快，或者没开硬件流控！
          return RES_ERROR;
        }

        while(HAL_SD_GetCardState(&hsd1) != HAL_SD_CARD_TRANSFER) { osDelay(1); }

        /* 4. 防旧数据：作废 Cache，读取真正的物理数据 */
        SCB_InvalidateDCache_by_Addr((uint32_t*)dma_scratch_buff, 512);

        memcpy(buff, dma_scratch_buff, 512);
        res = RES_OK;
...
DRESULT SD_write(BYTE lun, const BYTE *buff, DWORD sector, UINT count)
{
  ...
  for (UINT i = 0; i < count; i++)
  {
    memcpy(dma_scratch_buff, buff, 512);
    SCB_CleanDCache_by_Addr((uint32_t*)dma_scratch_buff, 512);

    if(BSP_SD_WriteBlocks_DMA((uint32_t*)dma_scratch_buff, sector + i, 1) == MSD_OK) {
...
{
  uint32_t msg = 0; // 失败也得唤醒，不然任务会死等 1000ms
  osMessageQueuePut(SDQueueID, &msg, 0, 0);
}
